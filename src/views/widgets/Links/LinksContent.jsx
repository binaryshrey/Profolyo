...
const LinksContent = () => {
  ...
  const { selectedWidget, updateCardTitle, profolyoEditorLayout, updateCardDescription, updateCardCoverImage, updateCardLinkURL } = EditorLayout();
  ...
  const getWidgetLinkURL = (widget) => {
    const component = profolyoEditorLayout[layoutMode].filter((item) => item.i === widget.i);
    return component[0]?.data?.link;
  };
  ...
  const uploadImage = async (selectedFile) => {
    ...
    try {
      ...
      const { data, error } = await supabase.storage.from('links').upload(fileName, selectedFile, {
        cacheControl: '3600',
        upsert: false,
      });
      if (error) throw error;
      updateCardCoverImage(selectedWidget.i, `${SUPABASE_STORAGE_URL}links/${fileName}`);
    ...
    <>
      <div className="w-full flex flex-col gap-2 ">
        ...
        <div className="pl-1 pr-1 pt-8">
          ...
          <EditorTextarea type="text" id="desc" placeholder="profolyo.me" maxLength="100" value={getWidgetDescription(selectedWidget)} onChange={() => updateCardDescription(selectedWidget.i, event.target.value)} />
        </div>

        <div className="pl-1 pr-1 pt-8">
          <Label htmlFor="desc">Link URL</Label>
          <EditorTextarea type="text" id="desc" placeholder="https://www.profolyo.me" maxLength="100" value={getWidgetLinkURL(selectedWidget)} onChange={() => updateCardLinkURL(selectedWidget.i, event.target.value)} />
        </div>
